...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const negocio = await prisma.dir_verde.findUnique({
    where: { id_negocio: Number(id_negocio) },
  });
  if (!negocio) {
    return {
      title: 'Negocio no encontrado | Negocios Verdes Cardique',
    };
  }
  return {
    title: `${negocio.negocio} | Negocios Verdes Cardique`,
    description: `${negocio.descripcion?.slice(0, 160)} - Negocio verde en ${negocio.direccion}.`,
    openGraph: {
      title: negocio.negocio,
      description: negocio.descripcion?.slice(0, 160),
      type: 'website',
    },
    robots: {
      index: true,
...
export default async function usePerfilNegocio({
  ...
  const negocioJsonLd = {
    ...
    address: {
      '@type': 'PostalAddress',
      addressLocality: negocio?.direccion,
      addressRegion: 'Bolívar',
      addressCountry: 'CO',
    },
    telephone: negocio?.whatsup,
    url: `https://TU-DOMINIO.com/verPerfilNegocio/${negocio?.id_negocio}`,
    priceRange: '$$',
  };
