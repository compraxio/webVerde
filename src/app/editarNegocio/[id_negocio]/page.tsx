...
type municipio = Prisma.municipiosGetPayload<object>;
...
export default function ActualizarNegocio() {
  ...
  useEffect(() => {
    ...
    async function cargarGruposYFases() {
      ...
      const municipios = await ConseguirTodosMunicipios();
      if (grupos.length === 0) {
        toast.error('añade un grupo primero');
        router.push('/');
      }
      if (municipios.length === 0) {
        toast.error('añade un municipio primero');
        router.push('/');
      }
      setGrupos(grupos);
      setFases(fases);
    ...
    cargarGruposYFases();
  }, [setGrupos, setFases, router, isAuthenticated]);
            ...
            <div className="flex flex-col gap-1.5 md:col-span-2">
              ...
              <select className={inputClass} id="municipio" {...register('id_municipio')}>
                ...
                {municipios.map((municipio) => (
                  <option key={municipio.cod_munic} value={municipio.cod_munic}>
                    {municipio.municipio}
                  </option>
                ))}
              </select>
              {errors.id_municipio?.message && <p>{errors.id_municipio.message}</p>}
            </div>
